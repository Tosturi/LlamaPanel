...
- `app/introspection.py` — `BinaryInspector`: запускает `llama-server --version`
  ...
  не изменится. Отдаётся через `GET /api/server/binary`. Это основа для формы
  флагов, которая строится по реально установленной версии llama.cpp, а не по
  зашитому списку. Формат `--help` зафиксирован в
  `tests/fixtures/llama_server_help.txt`.
